...
/**
 * 该接口用于导出某张表时，改变其数据库中的数据，实现该接口后，可以在自己的方法中判断表名、列名
 * 如果为需要的，将值改变后返回。该接口在数据库提取器中回调。如果不需要修改值，返回dbValue值
 * 这里只是非CLOB,BLOB数据
 * @author XieGuanNan
 * @date 2015-8-19-下午7:56:46
 */
public interface ExportDbValueChangeProcessor {

	public String changeDbValue(ExportMode mode,SynchPO synchPO,String dbValue,String colName) throws CallbackException;
}
